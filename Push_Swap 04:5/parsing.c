...
//     input = join_args_into_string(argc, argv);
//     if (!input)
//         return NULL;
//     nums = ft_split(input, ' ');
//     free(input);
...
//     while (nums[i])
//     {
//         if (!valid_num_check(nums[i]))
//             return (free_split(nums), free_on_error(stack), NULL);
//         num = ft_atol(nums[i]);
//         if (num < INT_MIN || num > INT_MAX || duplicate_num_check(stack, (int)num))
//             return (free_split(nums), free_on_error(stack), NULL);
//         ft_lstadd_back(&stack, ft_lstnew((int)num));
...
// char *join_args_into_string(int argc, char **argv)
// {
//     int i = 1;
...
//     {
//         temp = joined;
//         joined = ft_strjoin(joined, argv[i]);   // append current arg
//         free(temp);
